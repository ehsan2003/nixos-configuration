...
in
{
  ...
  text = ''
    ...
    bindsym --to-code XF86MonBrightnessUp exec ${pkgs.brightnessctl}/bin/brightnessctl set +5%
    bindsym --to-code XF86MonBrightnessDown exec ${pkgs.brightnessctl}/bin/brightnessctl set 5%-
    ...
    bindsym --to-code $mod+n exec --no-startup-id rofi -show 
    ...
    # bindsym --to-code $mod+b exec --no-startup-id "${ensure-class}/bin/ensure-class btop \"alacritty --class btop,btop -o font.size=8 -e btop\"";
    # bindsym --to-code $mod+s exec --no-startup-id "${ensure-class}/bin/ensure-class ai \"firefox https://duck.ai --class ai \" ;exec \"sleep 0.1; swaymsg scratchpad show\";
    ...
    # switch to workspace
    bindsym --to-code $mod+1 workspace number $ws1; exec "${ensure-class}/bin/ensure-class glrnvim glrnvim"
    bindsym --to-code $mod+2 workspace number $ws2; exec ${ensure-class}/bin/ensure-class firefox "firefox"
    bindsym --to-code $mod+3 workspace number $ws3; exec "${ensure-class}/bin/ensure-class Alacritty alacritty"
    bindsym --to-code $mod+4 workspace number $ws4; exec ${ensure-class}/bin/ensure-class aider "alacritty --class=aider"
    bindsym --to-code $mod+5 workspace number $ws5; exec "${ensure-class}/bin/ensure-class org.telegram.desktop Telegram"
    ...
    bindsym --to-code $mod+y exec "${notitrans-en}/bin/notitrans-en"
    bindsym --to-code $mod+g exec "${search-select}/bin/search-select"
    bindsym --to-code $mod+x exec "${notitrans-dict}/bin/notitrans-dict"

    bindsym --to-code $mod+c exec "${pkgs.dunst}/bin/dunstctl history-pop"
